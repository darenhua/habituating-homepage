...
export async function saveHabitEntry(
    entry: Omit<HabitEntry, "id" | "created_at" | "updated_at">,
): Promise<HabitEntry> {
    // Validate coding_level
    if (![0, 1, 2].includes(entry.coding_level)) {
        throw new Error(
            `Invalid coding_level: ${entry.coding_level}. Must be 0, 1, or 2.`,
        );
    }
    ...
    if (!dateRegex.test(entry.date)) {
        throw new Error(
            `Invalid date format: ${entry.date}. Must be yyyy-MM-dd.`,
        );
    }
